...
var fMassagePayloadGitlab = function(source, accessToken) {
    md = new MarkdownIt();
...
router.post('/challenges/:service?', function(req, res, next) {
    ...
    if (accessToken) {
        ...
        if (svcName == 'gitlab') {
            ...
            payload = fMassagePayloadGitlab(req.body, accessToken);
        } else {
            console.log('massaging payload for github');
            payload = fMassagePayload(req.body, accessToken);
        }
        ...
        console.log('No accessToken found in request header.');
        res.status(401).json({
            error: 'No accessToken found in request header.'
        })
    }
